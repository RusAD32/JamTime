/*  Cells counters are needed to prevent repetition when placing cells.                           */
/*  Cells of each type start from a random number and then are placed in a row.                   */
/*  When the number reaches max, in circles back to the beginning.                                */

var cells_counters = ds_map_create();
...
/* Which dead end room should come first...                                                       */
var dead_end = irandom(array_height_2d(cell_D) - 1);

var t_cross = irandom(array_height_2d(cell_RDL) - 1);
...
for(xx = 0; xx < dungeon_width; xx++)
{
    ...
    {   
        ...
        else if(ch == c_t.locked_finish or ch == c_t.locked_finishU or ch == c_t.locked_finishR or ch == c_t.locked_finishL)
        {
            cells = cell_locked_finish;
        }                     
        else if(ch == c_t.lock or ch == c_t.lockU or ch == c_t.lockR or ch == c_t.lockL)
        {
            cells = cell_lock;
            
            // TODO: If a key room exists, but lock does not, the lock room number should be the same
            // but % with the number of locked rooms            
        }        
        else if(ch == c_t.key or ch == c_t.keyU or ch == c_t.keyR or ch == c_t.keyL)
        {
            cells = cell_key;
            
            // TODO: If a lock room number exists, but key room does not, the key room number should be the same
            // but % with the number of key rooms
            
        }                
           
        else continue;
        
        if not ds_map_exists(cells_counters, ch) {
            cells_counters[? ch] = irandom(array_height_2d(cells) - 1);
        }
        else
        {
            cells_counters[? ch]++;
        ...
        cellno = cells_counters[? ch] % array_height_2d(cells);
